...
MAX_WAITING_JOBS = int(os.getenv("QUEUE_MAX_WAITING", "5"))
SYSTEM_CAPACITY = 1 + MAX_WAITING_JOBS
API_PORT = int(os.getenv("INFERENCE_PORT", "8000"))
METRICS_PORT = int(os.getenv("INFERENCE_METRICS_PORT", "9102"))
...
PROCESS_REQUESTS_TOTAL = Counter(
    "inference_process_requests_total",
    "Total number of requests to /process",
)
PROCESS_SUCCESS_TOTAL = Counter(
    "inference_process_success_total",
    "Total number of successfully processed PDFs",
)
PROCESS_ERRORS_TOTAL = Counter(
    "inference_process_errors_total",
    "Total number of processing errors",
)
PROCESS_QUEUE_REJECTED_TOTAL = Counter(
    "inference_process_queue_rejected_total",
    "Total number of requests rejected due to full queue",
)
QUEUE_LENGTH_GAUGE = Gauge(
    "inference_queue_length",
    "Current number of waiting jobs in inference queue",
)
WORKER_BUSY_GAUGE = Gauge(
    "inference_worker_busy",
    "Whether the single worker is busy: 0 or 1",
)
PROCESS_DURATION_SECONDS = Histogram(
    "inference_pdf_processing_duration_seconds",
    "Duration of PDF processing in inference",
...
class InMemorySequentialProcessor:
    def __init__(self, max_waiting_jobs: int = 5):
        self.max_waiting_jobs = max_waiting_jobs
        self.capacity = 1 + max_waiting_jobs
        self.wait_queue: "queue.Queue[QueueJob]" = queue.Queue()
    ...
    def system_load(self) -> Tuple[int, int]:
        with self.lock:
            busy = 1 if self.worker_busy else 0
            waiting = max(0, self.in_system - busy)
        return busy, waiting

    def try_submit(self, pdf_bytes: bytes, original_filename: str) -> Future:
        ...
        with self.lock:
            print(f"[QUEUE] before submit: in_system={self.in_system}, capacity={self.capacity}, worker_busy={self.worker_busy}")
            if self.in_system >= self.capacity:
                print("[QUEUE] reject 429")
                PROCESS_QUEUE_REJECTED_TOTAL.inc()
                raise HTTPException(status_code=429, detail="Очередь обработки переполнена. Попробуйте позже.")
            self.in_system += 1
            print(f"[QUEUE] accepted: in_system={self.in_system}")


            waiting = self.in_system - (1 if self.worker_busy else 0)
            QUEUE_LENGTH_GAUGE.set(max(0, waiting))

        self.wait_queue.put(job)
        QUEUE_LENGTH_GAUGE.set(self.wait_queue.qsize())
        return future
        ...
        while True:
            ...
            try:
                ...
                with self.lock:
                    self.worker_busy = False
                    self.in_system -= 1
                    WORKER_BUSY_GAUGE.set(0)
                    QUEUE_LENGTH_GAUGE.set(self.wait_queue.qsize())

    def _execute_job(self, job: QueueJob) -> Dict[str, Any]:
        with TemporaryDirectory(prefix=f"inference_{job.job_id}_") as tmp_dir_str:
            tmp_dir = Path(tmp_dir_str)
            safe_stem = Path(job.original_filename).stem[:100] or f"job_{job.job_id}"
            input_pdf_path = tmp_dir / f"{uuid4().hex}.pdf"
            output_xlsx_path = tmp_dir / f"{safe_stem}_tables.xlsx"

            input_pdf_path.write_bytes(job.pdf_bytes)

            try:
                with PROCESS_DURATION_SECONDS.time():
                    result_xlsx_path = process_pdf_file(
                ...
                if result_xlsx_path is None:
                    raise RuntimeError("Не удалось сформировать Excel-файл: таблицы не найдены или экспорт не выполнен.")

                result_xlsx_path = Path(result_xlsx_path)
                if not result_xlsx_path.exists():
                    raise RuntimeError("Backend завершил обработку, но выходной xlsx-файл не найден.")

                result_bytes = result_xlsx_path.read_bytes()

                PROCESS_SUCCESS_TOTAL.inc()
                return {
...
processor = InMemorySequentialProcessor(max_waiting_jobs=MAX_WAITING_JOBS)
...
def resize_keep_aspect(img: Image.Image, max_long_side: int) -> Image.Image:
    ...
    np_arr = np.array(img)
    resized_np = cv2.resize(np_arr, (new_w, new_h), interpolation=cv2.INTER_AREA)
...
def load_doc_orientation_model():
    ...
    DOC_ORI_MODEL = DocImgOrientationClassification(
        model_name="PP-LCNet_x1_0_doc_ori",
        device=get_paddle_device(),
        model_dir=get_model_dir("PP-LCNet_x1_0_doc_ori"),
    )
    DOC_ORI_MODEL.predict = TimeIt("ORI_predict")(DOC_ORI_MODEL.predict)
    return DOC_ORI_MODEL
...
def load_layout_model():
    ...
    LAYOUT_MODEL = LayoutDetection(
        ...
        device=get_paddle_device(),
        model_dir=get_model_dir("PicoDet_layout_1x_table"),
    )
    LAYOUT_MODEL.predict = TimeIt("TD_predict")(LAYOUT_MODEL.predict)
    return LAYOUT_MODEL
...
def load_ocr_det_model():
    ...
    OCR_DET_MODEL = TextDetection(
        model_name="PP-OCRv5_mobile_det",
        device=get_paddle_device(),
        model_dir=get_model_dir("PP-OCRv5_mobile_det"),
    )
    OCR_DET_MODEL.predict = TimeIt("DET_predict")(OCR_DET_MODEL.predict)
    return OCR_DET_MODEL
...
def load_ocr_rec_model():
    ...
    OCR_REC_MODEL = TextRecognition(
        model_name="cyrillic_PP-OCRv3_mobile_rec",
        device=get_paddle_device(),
        model_dir=get_model_dir("cyrillic_PP-OCRv3_mobile_rec"),
    )
    return OCR_REC_MODEL
...
def classify_page_orientations(images: List[Image.Image]) -> List[int]:
    ...
    inputs: List[np.ndarray] = [np.array(img.convert("RGB")) for img in images]
    orientations: List[int] = []

    outputs = model.predict(input=inputs, batch_size=BATCH_SIZE_DET)
    class_id_to_angle = {0: 0, 1: 90, 2: 180, 3: 270}
    ...
    if len(orientations) != len(images):
        orientations = orientations[: len(images)] + [0] * max(0, len(images) - len(orientations))
...
def pdf_to_images(
...
) -> Tuple[List[PageImage], List[float]]:
    ...
    with TimeIt("Parallel PDF rendering"):
        raw_images = convert_from_path(
            pdf_path,
            dpi=dpi,
            fmt="RGB",
            thread_count=PDF_CONVERT_THREAD_COUNT,
            use_cropbox=True,
    ...
    results: Dict[int, Tuple[PageImage, float]] = {}

    with TimeIt("Parallel render preprocessing"):
        with ThreadPoolExecutor(max_workers=PDF_CONVERT_THREAD_COUNT) as pool:
            futures = {
...
def build_annotation_with_table_ocr(pdf_path, run_rec=True, first_page=1, last_page=None):
    ...
    for t in base["tables"]:
        ...
        ocr_bbox_rel = [
            float(real_bbox_rel[0]),
            max(0.0, float(real_bbox_rel[1]) - 0.10),
            float(real_bbox_rel[2]),
            float(real_bbox_rel[3]),
        ]
    ...
    det_boxes_by_table_id = run_ocr_det_on_table_crops(table_inputs) if len(table_inputs) > 0 else {}
...
def build_conservative_table_groups(annotation_dict):
    ...
    for class_id, class_tables in by_class.items():
        ...
        candidate_pages = sorted(p for p, items in page_to_tables.items() if len(items) == 1)
...
def export_table_pipeline_results_to_excel(tsr_candidates, pipeline_results, pdf_path, final_xlsx_path, working_dir):
    class_to_sheet = {
        0: "ОФП",
        1: "ОФР",
        2: "ОДДС",
    ...
    for cand, res in zip(tsr_candidates, pipeline_results):
        ...
        if out_path.is_file():
            ...
            for p in out_path.glob("*.xlsx"):
                if p.is_file():
                    real_xlsx = p
                    break

        if real_xlsx is None:
            continue

        saved_files[class_id].append((cand, real_xlsx))
    ...
    for class_id in sorted(saved_files.keys()):
        ...
        for _, xlsx_path in saved_files[class_id]:
            try:
...
def merge_annotation_chunks(
    ...
    merged_num_pages = max(int(accumulated_annotation.get("num_pages", 0)), chunk_num_pages)
    ...
    for page_meta in chunk_pages_meta:
        ...
        if page_index not in pages_meta_by_index:
            pages_meta_by_index[page_index] = {"page_index": page_index, "ocr_rows": remapped_rows}
        else:
    ...
    for page_index in sorted(pages_meta_by_index.keys()):
        ...
        page_meta["ocr_rows"].sort(key=lambda row: (float(row["bbox"][1]), float(row["bbox"][0])))
...
def check_tsr_ready_conditions(annotation_dict: dict, last_processed_page: int, target_classes: Tuple[int, ...] = (0, 1, 2, 3)) -> dict:
    ...
    for table in tables:
        ...
        if page_index == int(last_processed_page):
            target_tables_on_last_page.append(
                {"table_id": table_id, "page_index": page_index, "class_id": class_id}
            )
...
def prune_cached_images(annotation_dict: dict, cached_pages: Dict[int, PageImage], target_classes: Tuple[int, ...] = (0, 1, 2, 3)) -> Dict[int, PageImage]:
    if not cached_pages:
        gc.collect()
        return {}
...
def run_pipeline_incremental_until_ready(
    ...
    tsr_candidates = build_conservative_table_groups(accumulated_annotation)
    pipeline_results = []
    xlsx_path = None

    if len(pages_for_tsr) > 0:
        built_candidates = build_tsr_candidates(accumulated_annotation, pages_for_tsr)
        if len(built_candidates) > 0:
            pipeline_results = run_table_pipeline_on_candidates(built_candidates)
            xlsx_path = export_table_pipeline_results_to_excel(
                tsr_candidates=built_candidates,
...
def build_tsr_candidates(annotation_dict, pages):
    ...
    for group_idx, group in enumerate(groups):
        ...
        final_image = table_crops[0] if len(table_crops) == 1 else stack_images_vertically(table_crops)
...
def process_pdf_file(input_pdf_path: Path, output_xlsx_path: Path, working_dir: Path, log):
    log("Обработка файла...")
    try:
        result = run_pipeline_incremental_until_ready(
            pdf_path=input_pdf_path,
            final_xlsx_path=output_xlsx_path,
            working_dir=working_dir,
        )
...
def cleanup_runtime_objects():
    global TABLE_PIPELINE, DOC_ORI_MODEL, OCR_DET_MODEL, OCR_REC_MODEL, LAYOUT_MODEL, TABLE_CLS_MODEL, TABLE_CLS_STOI, TABLE_CLS_ROW_CODES
...
def process_endpoint(file: UploadFile = File(...)):
    ...
    if not filename.lower().endswith(".pdf"):
        raise HTTPException(status_code=400, detail="Допустим только PDF-файл.")

    pdf_bytes = file.file.read()
    if not pdf_bytes:
        raise HTTPException(status_code=400, detail="Передан пустой файл.")

    future = processor.try_submit(pdf_bytes=pdf_bytes, original_filename=filename)
...
def start_metrics_server_once():
    thread = threading.Thread(target=lambda: start_http_server(METRICS_PORT), daemon=True)
    thread.start()
